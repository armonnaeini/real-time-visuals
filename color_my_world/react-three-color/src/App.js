...
import { uniforms, vertexShader, fragmentShader } from "./shaders2";
...
const uniform2 = {
  ...
  uSpeed: { type: "f", value: 0.25 },
  uNoiseDensity: { type: "f", value: 1.25 },
  uNoiseStrength: { type: "f", value: 0.95 },
  uFrequency: { type: "f", value: 16.0 },
  uAmplitude: { type: "f", value: 0.15 },
  uIntensity: { type: "f", value: 16 }
};
...
const Sphere = (props) => {
  ...
  useFrame(() => {
    ...
    // uniform2.uTime.value = Math.random() * 0.02;
    // uniform2.uTime.value = t = (t + 0.01) % 1;
...
const Scene = () => {
  ...
  //scene.background = new Color(0x000000, 0);

  return (
    <>
      <Sphere position={[0, 0, 1.75]} />
    </>
  );
};
...
export default function App() {
  return (
    <Canvas
      ...
      style={{ height: 900 }}
      onCreated={(state) => state.gl.setClearColor(0xff0000, 0)}
    >
      <Scene />
    </Canvas>
  );
